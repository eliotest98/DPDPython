...
class TypeDetector:
    ...
    def __init__(self, system_object, terminal):
        ...
        self.imports_detector = ImportsDetector(system_object, terminal)
        self.detect_type_with_call_functions(system_object, terminal)
        self.detect_type_of_variables_classes(system_object)

    def detect_type_with_call_functions(self, system_object, terminal):
        for i in range(system_object.get_class_number()):
            self.progressor.update(len(system_object.get_class_names()), i,
                                   "TypeDetector detect type with call functions", terminal)
                ...
                if isinstance(variable, VariableObject):
                    ...
                    if isinstance(call_function, CallFunctionObject):
                        ...
                        if system_object.get_class_object_with_class_name(
                                origin_class + "." + str(call_function.get_method_name())) is not None:
                            ...
                            for single_class in class_or_file_object.get_class_list():
                                if single_class.get_class_name() == call_function.get_method_name():
                                    variable.set_type(origin_class + "." + str(call_function.get_method_name()))
                                    if key not in self.variable_scope:
                                        scope = Scope()
                                        scope.set_class_name(key)
                                        self.variable_scope[key] = scope
